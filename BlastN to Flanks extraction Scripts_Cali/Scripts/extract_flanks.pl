...
#########################################################
#
# Platform: NCI Gadi HPC (this perl script can be run on any Linux CLI)
#
# Description: 
...
#	- Extracts the 400 bp flanks (200 bp from either side of IS insertion site) and 
#	 writes flank multifasta for each IS
#	- Only unique flanks are kept; for IS with multiple identical flank sequences, 
#	 the fasta header is retained for the first and the number of duplicates is recorded at the end of the header
# 	- Requires blast+ to be loaded to extract fasta from NCBI-formatted nt/nr database
#	- Requires a pre-formatted blast nr/nt database
...
# 	- Script considers if the hit was plus or minus strand: If the subject start is greater than the subject end,
#	 the concatenated flanks are reverse-complimented and 'RC' is added to the fasta header
#	- There are no input arguments required; flank size of 200 is hard-coded under flank_size variable
#
# Usage:
#       - This script is executed by extract_flanks.pbs
#
# Compute resources:
#       - 1 CPU, 6.5 hours
...
my $filtered_hits = "./Output/$dataset\.bacterial.blast.filtered";
my $nt = '/g/data/er01/NCBI/preformatted_2022_11_21/nt'; # nt/nr BLAST database 

# Output files:
my $outdir = './Output/Flanking_fastas';
`mkdir -p $outdir`;

my $summary = "$outdir\/$dataset\.bacterial.blast.filtered.counts.txt";
open (S, ">$summary") || die "$! write $summary\n";
print S "#IS\tFiltered_hits\tUnique_flanks\n"; 
...
while (my $line = <F>) {
	...
	# Run blastdbcmd utility to obtain fasta of the subject sequence for each of the 2 flanks
	my $accession = $cols[7];
	...
	if ($query eq $last_query) {
		...
		$flank_fasta = "./$outdir\/$last_query\_400bp_flanks.fasta.temp"; 
		create_multi_fasta ($query_concat_list, $flank_fasta);
		
		# Keep only unique sequences:
		$unique_flank_fasta = "./$outdir\/$last_query\_400bp_unique_flanks.fasta";
		my ($total_flanks, $unique_flanks) = unique_flanks ($flank_fasta, $unique_flank_fasta); 
		
		# Add counts to report file:
		print S "$last_query\t$total_flanks\t$unique_flanks\n"; 

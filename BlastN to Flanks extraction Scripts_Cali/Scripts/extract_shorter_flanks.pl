...
my $filter_report = "Output/IS110_complete_Ident95_E0.bacterial.blast.report";  

# Output files:
my $outdir = "./Output/Flanking_fastas_${flank_size}bp_$filterName";
`mkdir -p $outdir`;
my $warnings = "$outdir\/$dataset\_${flank_size}bp_$filterName.failed.txt"; 
...
chomp (my $header = <F>);
my ($count, $ok, $warn_out, $warn_in) = 0; 
while (my $line = <F>) {
        chomp $line;

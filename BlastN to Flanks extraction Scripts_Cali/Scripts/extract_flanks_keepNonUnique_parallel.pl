...
# Usage:
# This takes all filtered BLAST hits per single IS and executes
# parallel by IS, to speed up execution
# First, make inputs: extract_flanks_parallel_make_inputs.pl
# Then submit with: extract_flanks_run_parallel.pbs
# 
#
...
# to name input/output files, ie this must match previous step  
...
my $nt = '/g/data/er01/NCBI/preformatted_2022_11_21/nt'; 
  
#########################################################

# Input file: 
my $filtered_hits = $ARGV[0]; # a shorter list of hits, each with unique chunk number 
chomp $filtered_hits; 
$filtered_hits =~ m/\-(\d+)$/; 
my $chunk_num = $1; 

# Output files:
my $temp_dir = "$JOBFS\/$chunk_num"; 
my $final_outdir = "./Output/Flanking_fastas_$filterName";
`mkdir -p $temp_dir $final_outdir`;

my $log = "$final_outdir/$chunk_num\.statuslog"; 
open (LOG, ">$log") || die "$! write $log\n"; 
my $datestring = localtime();
print LOG "Local date and time $datestring\n";
print LOG "Started $filtered_hits writing temp to $temp_dir and out to $final_outdir\n\n"; 
...
#chomp (my $header = <F>); # No header on the split hits chunk files!
...
while (my $line = <F>) {
	...
	my $accession = $cols[7];
	my $out_left = "$temp_dir\/$query\_within_$accession\_leftFlank_$left_flank_start\-$left_flank_end\.fasta.temp"; 
	my $out_right = "$temp_dir\/$query\_within_$accession\_rightFlank_$right_flank_start\-$right_flank_end\.fasta.temp";   
	...
	my $header = "\>".$outname;
	my $concat = "$temp_dir\/$outname\.fasta.temp";
	#`rm -rf $concat`; 1 
	...
	if ($query eq $last_query) {
		...
		$flank_fasta = "./$final_outdir\/$last_query\_400bp_flanks.fasta-$chunk_num"; 
		create_multi_fasta ($query_concat_list, $flank_fasta);
		
		# Cleanup temp files: 
		cleanup ($temp_dir, $last_query); 
